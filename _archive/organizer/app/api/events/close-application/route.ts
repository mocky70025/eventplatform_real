...
export async function POST(request: NextRequest) {
  try {
    const { eventId, organizerId } = await request.json()
    ...
    if (event.is_application_closed) {
      return NextResponse.json(
    ...
    if (updateError) {
      return NextResponse.json(
        { error: 'Failed to close application', details: updateError.message },
        { status: 500 }
      )
    }

    // 申し込みデータを整形
    const exhibitorData: ApplicationData[] = (applications || []).map((app: any) => ({
      id: app.id,
    ...
    return NextResponse.json({
      ...
      applications: exhibitorData,
      count: exhibitorData.length
    })
  } catch (error: any) {
